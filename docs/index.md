...
-> This provider is in "early access", issues are to be expected, please report any you find as a Github issue. Please see the current status over at: https://github.com/echowings/terraform-provider-vyos-rolling/milestones?state=open

> **Note on log masking and secrets**
>
> The provider masks the configured `endpoint` and `api_key` values in logs using regular expressions. To avoid any chance of crashes in the underlying regex engine and to treat these values purely as literals, the provider internally escapes them before compiling the patterns. This means that even if your API key or endpoint contains regex-special characters, they are still safely masked and will **not** appear in provider logs.
